...
# Training models with `pyannote-audio-train`

This tutorial assumes that you have already prepared the AMI corpus following the instructions available [here](https://github.com/pyannote/AMI-diarization-setup).

```bash
export PYANNOTE_DATABASE_CONFIG=/path/to/AMI-diarization-setup/pyannote/database.yml
export PROTOCOL=AMI.SpeakerDiarization.only_words
pyannote-database info $PROTOCOL
```
...
```bash
export PYANNOTE_DATABASE_CONFIG=/path/to/AMI-diarization-setup/database.yml
pyannote-audio-train \
    #
    protocol=AMI.SpeakerDiarization.only_words \
    task=Segmentation task.duration=5. task.batch_size=32 task.num_workers=10 \
    model=PyanNet model.sincnet.stride=10 model.lstm.num_layers=4 \
...
```bash
pyannote-audio-train --config-dir=/path/to/config \
    model=YourModel +model.param1=
